(function(window, graphit, console, undefined) {
	...
	function Cgraphit_message(options) {
		options = options || {};
		options.className = 'graphit/message';
	...
	graphit.export('graphit/message', Cgraphit_message);
	
	/**
	 * Module is responsible to record points and send data to the tools when
	 * mouse is pressed
	 * 
	 * @param {Hash}
	 ...
	 */
	Module.prototype.reset_index = function() {
		this.index = 0;
	};
	 ...
	 * @private
	 * @param {Object}
	 *            d Data passed to our grapher
	 * @returns {Boolean}
	 */
	Module.prototype._graph = function(cMessage) {
	...
	Module.prototype.stop = function() {
		try {
			...
			if (!cTool.post_graph(cMessage, x, y, width, height, 0, 0, width,
					height)) {
				cLayer.drawImage(cPrefrag.cCanvas.data, x, y, width, height, 0,
						0);
			}
			// cLayer.redraw();
			// cs.redraw();

(function(window, graphit, console, undefined) {
	 ...
	 * @param layer
	 */
	Module.prototype.set_current_layer = function(layer) {
		this.layer_manager.select(layer);
	};

	Module.prototype.update_grid = function() {
	 ...
	 * selected layer so our rendering process a finite number of layers -
	 * bottom - selected - prefrag - up - grid
	 * 
	 * @param force
	 * @returns {Boolean}
	 */
	Module.prototype.redraw = function(force, dcanvas) {
		...
		if (this.layer_manager.special_layers.stack_down != undefined) {
			tctx.drawImage(
					this.layer_manager.special_layers.stack_down.cCanvas.data,
		...
		tctx.drawImage(this.layer_manager.special_layers.prefrag.cCanvas.data,
				0, 0, canvas.width, canvas.height);
		if ('down_composite_operation' in this.layer_manager.special_layers.prefrag) {
		...
		if (this.layer_manager.special_layers.stack_up != undefined) {
			tctx.drawImage(
					this.layer_manager.special_layers.stack_up.cCanvas.data, 0,
	 ...
	 * @returns {Boolean}
	 */
	Module.prototype.callback_mousedown = function(e, obj) {
	 ...
	 * @returns {Boolean}
	 */
	Module.prototype.callback_mouseup = function(e, obj) {
		if (!this.cMouse.is_pushed()) {
	 ...
	 * @param obj
	 */
	Module.prototype.callback_mousemove = function(e, obj) {
	 ...
	 */
	Module.prototype.save_as_json = function() {
		// var data = {
		// label : this.label,
		// layers : [],
		// };
